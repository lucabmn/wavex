...
export function ConnectionsPage() {
  ...
  return (
    <>
      <Heading title="This machine" first />
      <Row
      ...
      <Heading title="Paired hosts" />
      {hosts.length === 0 ? (
        <Row
          label="No hosts yet"
          description="Paste a connection code from another machine's wavex to add one."
        />
      ) : null}
      {hosts.map((host) => {
          ...
          <Row
            ...
            label={
              <span className="flex items-center gap-2">
                ...
                <span className={`shrink-0 text-[11px] ${PHASE_TONE[phase] ?? "text-content/40"}`}>
      ...
      {error ? (
        <p role="alert" className="pt-4 text-[12px] text-red-300/80">
          {error}
        </p>
      ) : null}
    </>
