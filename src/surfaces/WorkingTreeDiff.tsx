...
export function WorkingTreeDiff({ cwd, focusPath }: Props) {
    ...
    return files.map((file) => {
      ...
      const unified =
        loaded && !loaded.binary && !loaded.tooLarge
          ...
          : null;
      const unchanged =
        unified != null && unified.additions === 0 && unified.deletions === 0 && !loaded?.binary;
      return {
        id: file.relative,
        path: file.path,
        label: file.relative,
  ...
  if (!cwd || cwd === "~") {
    return (
      <p className="grid h-full place-items-center text-[13.5px] text-content/45">
        No project folder
      </p>
    );
  }
  if (error) {
    return (
      <div className="grid h-full place-items-center p-6 text-center">
        ...
        <p className="mt-1 text-[12.5px] text-content/50">{error}</p>
      </div>
    );
  }
  if (files == null) {
