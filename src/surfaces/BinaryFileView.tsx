...
function ImageView({ url, size, mime }: { url: string; size: number; mime: string }) {
    ...
    <div className="flex h-full min-h-0 flex-col">
      ...
      >
        <img
          ...
          className={
            zoom === "fit" ? "max-h-full max-w-full object-contain" : "max-w-none object-contain"
          }
          style={
            zoom === "fit" || !natural
              ? undefined
      ...
      <footer className="flex h-8 shrink-0 items-center gap-3 border-t border-content/10 px-3 text-[11px] text-content/50">
        ...
        <ZoomButton
          label="Zoom out"
          onClick={() => setZoom((value) => clampZoom((value === "fit" ? 1 : value) / 1.5))}
        >
