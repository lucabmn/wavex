...
export function isVirtualDocumentTab(file: FilePaneTab): boolean {
  return isPlanTab(file) || isReleaseNotesTab(file) || isCommitTab(file) || isReferencesTab(file);
}
...
export function openChangesTab(tab: WorkspaceTab, cwd: string, focusPath?: string): WorkspaceTab {
  tab = isolateTerminalPanes(tab);
  const next = newChangesTab(cwd, focusPath);
  const existingPane = tab.editorPanes.find((pane) => pane.files.some(isChangesTab));
    ...
    editorPanes: opened.editorPanes.map((pane) =>
      pane.files.some(isChangesTab)
        ? {
            ...
            activeFileId: pane.files.find(isChangesTab)?.id ?? pane.activeFileId,
          }
        : pane,
    ),
  };
