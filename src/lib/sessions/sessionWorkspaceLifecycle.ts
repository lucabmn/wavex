...
} from "../workspace/workspaceTabGroups";
...
export function removeSessionFromWorkspace({
  ...
  for (const original of tabs) {
    ...
    const remainingConversations = leafIds(tab.layout).some((id) => remainingSessionIds.has(id));
...
function removeSessionDocuments(tab: WorkspaceTab, sessionId: string): WorkspaceTab | null {
  if (
    !tab.editorPanes.some((pane) => pane.files.some((file) => file.plan?.sessionId === sessionId))
  )
    return tab;
  ...
  for (const pane of tab.editorPanes) {
    const files = pane.files.filter((file) => file.plan?.sessionId !== sessionId);
    if (files.length > 0) {
      editorPanes.push({
        ...pane,
        files,
