...
export function foldUnifiedLines(
  ...
  while (index < lines.length) {
    ...
    if (line.kind === "context" && !visible[index]) {
      const start = index;
      while (index < lines.length && lines[index].kind === "context" && !visible[index]) {
        index += 1;
      }
      blocks.push({
        kind: "fold",
...
function linesInRange(doc: Text, from: number, to: number): { text: string; number: number }[] {
...
function visibleContext(lines: readonly UnifiedLine[], context: number): boolean[] {
