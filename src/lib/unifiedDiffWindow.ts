import { revealedFold, type FoldReveal, type UnifiedBlock, type UnifiedLine } from "./unifiedDiff";
...
export function flattenVisibleRows(
  ...
  for (const block of blocks) {
    if (block.kind === "fold") {
      ...
      if (split.tail > 0) {
        pushLines(rows, block.lines.slice(block.lines.length - split.tail), false);
      }
      continue;
    }
    for (const line of block.lines) {
      const pos = line.pos ?? block.pos;
      const stage = canStageHunk && (line.kind === "add" || line.kind === "del") && pos != null;
      rows.push({
        type: "line",
...
function pushLines(rows: DiffViewRow[], lines: readonly UnifiedLine[], canStage: boolean) {
  for (const line of lines) {
    const stage = canStage && (line.kind === "add" || line.kind === "del") && line.pos != null;
    rows.push({
      type: "line",
      line,
      stage,
