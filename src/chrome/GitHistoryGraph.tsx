...
import { gitHistory, subscribeGitChanged, type GitHistoryCommit } from "../lib/fs";
import {
  GRAPH_ROW_PX,
  historyItemGraph,
  layoutGitGraph,
...
function RefPill({ refInfo }: { refInfo: GraphRef }) {
  ...
  return (
    ...
    >
      {local ? <GitBranch className="size-2.5 shrink-0" strokeWidth={2} /> : null}
      <span className="min-w-0 truncate">{refInfo.name}</span>
    </span>
  );
}

function useGitHistory(cwd: string, enabled: boolean): { commits: GitHistoryCommit[] } {
  const [commits, setCommits] = useState<GitHistoryCommit[]>(() => historyByCwd.get(cwd) ?? []);
...
function sameHistory(prev: GitHistoryCommit[], next: GitHistoryCommit[]): boolean {
  ...
  return prev.every((commit, i) => {
    ...
    return (
      ...
      commit.refs.every(
        (ref, j) => other.refs[j]?.name === ref.name && other.refs[j]?.kind === ref.kind,
      )
    );
  });
}
