...
function ApprovalToastCard({
  ...
  return (
    <article
      className="approval-toast pointer-events-auto overflow-hidden rounded-xl border border-edge-strong border-dashed bg-content/10 shadow-xl backdrop-blur-xl"
      role="status"
    >
      <button
        type="button"
        onClick={openSession}
        className="flex w-full flex-col gap-2 px-3.5 py-3 text-left hover:bg-hover"
      >
      ...
      {notice.kind === "question" ? null : (
        <div className="flex gap-2 border-t border-edge px-3.5 py-2.5">
          <button
            type="button"
            className="flex-1 rounded-md ui-fill px-2.5 py-1 text-[11px] font-medium"
          ...
          <button
            type="button"
            className="flex-1 rounded-md bg-content/10 px-2.5 py-1 text-[11px] font-medium text-content/70 hover:bg-hover"
