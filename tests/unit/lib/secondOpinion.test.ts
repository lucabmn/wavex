...
describe("turn extracts", () => {
  it("reads the user request, report, and edited files", () => {
    ...
    expect(turnReport(turn)).toBe("I'll patch UsageFooter.\n\n## Plan\n\n- edit the chip\n\nDone.");
    expect(turnEditedFiles(turn, "/repo")).toEqual(["src/chrome/UsageFooter.tsx"]);
  });

  it("dedupes edited paths", () => {
    expect(turnEditedFiles([edit("a", "src/App.tsx"), edit("b", "src/App.tsx")], "/repo")).toEqual([
      "src/App.tsx",
    ]);
  });
});
...
describe("secondOpinionTargets", () => {
  const all: HarnessId[] = ["claude", "codex", "cursor", "opencode", "pi", "omp", "fx", "grok"];
