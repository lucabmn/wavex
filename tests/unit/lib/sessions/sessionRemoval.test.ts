...
import type { SessionWorkspaceRemoval } from "@/lib/sessions/sessionWorkspaceLifecycle";
import { runSessionRemoval } from "@/lib/sessions/sessionRemoval";
import { deleteSession, setSessionArchived, upsertSession } from "@/lib/sessions/sessionStore";

const mocks = vi.hoisted(() => ({ invokeOn: vi.fn() }));
vi.mock("@/lib/transport", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/transport")>();
  return { ...actual, invokeOn: mocks.invokeOn };
});
...
function fixture(mode: "archive" | "delete") {
  ...
  mocks.invokeOn.mockImplementation(
    async (_hostId: string, command: string, args?: Record<string, unknown>) => {
      if (command === "session_upsert") {
        const session = args?.["session"] as Session | undefined;
        return { ...session, createdAt: 1, updatedAt: 1 };
      }
    },
  );
  return {
    ...
    run: () =>
      runSessionRemoval({
        ...
        updateSession: (stopped) => {
          state = {
            ...state,
            sessions: state.sessions.map((s) => (s.id === stopped.id ? stopped : s)),
          };
        },
...
afterEach(() => mocks.invokeOn.mockReset());
...
describe.each(["archive", "delete"] as const)("%s lifecycle", (mode) => {
  it("preserves another agent's completion, new tabs, and focus across both waits", async () => {
    ...
    confirm.resolve();
    await vi.waitFor(() => expect(mocks.invokeOn).toHaveBeenCalled());
  ...
  it("retains a stopped session when storage fails", async () => {
    const f = fixture(mode);
    mocks.invokeOn.mockRejectedValue(new Error("disk unavailable"));
    await expect(f.run()).rejects.toThrow("disk unavailable");
    expect(f.stop).toHaveBeenCalledOnce();
    expect(f.commit).not.toHaveBeenCalled();
    expect(f.read().sessions[0]).toMatchObject({ busy: false });
  ...
  it("does not stop or write anything if confirmation is declined", async () => {
    ...
    expect(mocks.invokeOn).not.toHaveBeenCalled();
...
it("archives the flushed transcript after cancellation, with streaming stopped", async () => {
  ...
  await f.run();
  const [, command, args] = mocks.invokeOn.mock.calls[0];
  expect(command).toBe("session_upsert");
  expect(args.session.blocks.at(-1)).toMatchObject({ text: "last buffered output" });
  expect(args.session.blocks.at(-1).streaming).toBeFalsy();
  expect(mocks.invokeOn.mock.calls.map(([, name]) => name)).toEqual([
    "session_upsert",
