...
describe("githubAvatarUrl", () => {
  it("builds the GitHub avatar URL", () => {
    expect(githubAvatarUrl("maya")).toBe("https://avatars.githubusercontent.com/maya?s=64");
  });
...
describe("inboxPersonAvatarUrl", () => {
  it("prefers an explicit avatar", () => {
    expect(inboxPersonAvatarUrl("github", "maya", "https://cdn.example/maya.png")).toBe(
      "https://cdn.example/maya.png",
    );
  });

  it("falls back to GitHub avatars", () => {
...
describe("formatRelativeTime", () => {
  it("formats hours ago", () => {
    ...
    expect(formatRelativeTime("2026-08-27T10:00:00Z", now, "en")).toBe("2 hours ago");
  });
...
describe("githubReviewDecisionLabel", () => {
  it("labels GitHub review rollups", () => {
    ...
    expect(githubReviewDecisionLabel("changes_requested")).toBe("Changes requested");
...
describe("inboxListCacheKey", () => {
  it("separates assigned-to-me from the unfiltered query", () => {
    ...
    expect(inboxListCacheKey(projects, base)).not.toBe(
      inboxListCacheKey(projects, { ...base, assignedToMe: true }),
    );
  });
});
...
describe("dedupeInboxItems", () => {
  it("keeps one card per GitHub issue across local checkouts", () => {
    ...
    expect(inboxItemKey(deduped[0]!)).toBe("github:lucabmn/wavex:issue:10");
  });
});
...
describe("groupProjectsByRepo", () => {
  it("fetches each GitHub remote once", () => {
    expect(
      groupProjectsByRepo([
        { path: "/tmp/wavex", repo: "lucabmn/wavex" },
        { path: "/tmp/agent-terminal", repo: "LucaBmn/wavex" },
...
describe("uniqueInboxProjects", () => {
  it("drops duplicate paths", () => {
    expect(
      uniqueInboxProjects([{ path: "/tmp/web/" }, { path: "/tmp/web" }, { path: "/tmp/docs" }]),
...
describe("filterInboxItems", () => {
  ...
  it("matches title, number, kind, repo, and labels", () => {
    expect(filterInboxItems(items, "checkout").map((row) => row.number)).toEqual([12]);
    expect(filterInboxItems(items, "#4").map((row) => row.number)).toEqual([4]);
    expect(filterInboxItems(items, "pull").map((row) => row.number)).toEqual([12]);
    expect(filterInboxItems(items, "docs").map((row) => row.number)).toEqual([4]);
    expect(filterInboxItems(items, "bug").map((row) => row.number)).toEqual([12]);
  });
});
...
describe("inboxItemKey", () => {
  it("keys items by repo, kind, and number", () => {
    expect(
      inboxItemKey(
        item({
          number: 9,
