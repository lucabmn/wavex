...
describe("composerRunner", () => {
  ...
  it("jumps a parabola over the hurdle and lands after it", () => {
    ...
    expect(jumpHeight(mid, obstacle)).toBeGreaterThan(jumpHeight(obstacle.left, obstacle));
  ...
  it("collects only when the sprite overlaps the coin", () => {
    ...
    expect(coinCollected({ x: 120, y: 8, facing: 1, airborne: true }, coins[0])).toBe(false);
    expect(coinCollected({ x: 40, y: peak.y, facing: 1, airborne: true }, coins[0])).toBe(false);
  });
  ...
  it("orbits pixel stars around the sprite for the stun, then clears them", () => {
    ...
    expect(new Set(start.map((star) => `${star.dx},${star.dy}`)).size).toBe(STAR_COUNT);
