...
describe("buildPiSpawnArgs", () => {
  it("starts RPC without stripping the user's extensions", () => {
    ...
    expect(buildPiSpawnArgs(PI_FLAVOR, { model: "anthropic/claude-sonnet-4" })).toEqual([
      "--mode",
      "rpc",
      "--model",
  ...
  it("strips extensions for throwaway catalog probes", () => {
    expect(buildPiSpawnArgs(PI_FLAVOR, { noSession: true, noExtensions: true })).toEqual([
      "--mode",
      "rpc",
      "--no-session",
      "--no-extensions",
...
describe("parsePiVersion", () => {
  it("reads a semver from CLI output", () => {
    ...
    expect(parsePiVersion("@earendil-works/pi-coding-agent/0.30.1")).toBe("0.30.1");
  });
});
...
describe("tools and models", () => {
  ...
  it("keeps earlier tool args when a later update is partial", () => {
    expect(mergeToolInput({ command: "git status -s" }, { timeout: 30 })).toEqual({
  ...
  it("reads session and context stats", () => {
    ...
    expect(contextFromUsage({ type: "message_end", message: { role: "user" } }, 200000)).toBeNull();
    expect(
      contextFromUsage(
        {
          type: "message_end",
