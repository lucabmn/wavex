...
describe("parseServerUrlFromOutput", () => {
  it("reads the listening URL from server output", () => {
    expect(parseServerUrlFromOutput("opencode server listening on http://127.0.0.1:4096")).toBe(
      "http://127.0.0.1:4096",
    );
  });
});
...
describe("OpenCode CLI inventory parsers", () => {
  it("parses models --verbose output", () => {
    ...
    expect(models[1].settings?.some((setting) => setting.id === "variant")).toBe(true);
    expect(models[0].settings?.find((setting) => setting.id === "agent")?.value).toBe("build");
  });
...
describe("OpenCode helpers", () => {
  it("ignores OpenCode placeholder titles", () => {
    expect(isOpenCodeDefaultTitle("New session - 2026-08-16T07:24:01.000Z")).toBe(true);
  ...
  it("detects 404 / NotFoundError", () => {
    ...
    expect(isOpenCodeNotFound({ status: 500, name: "NotFoundError" })).toBe(false);
  });

  it("infers default variant and agent", () => {
    expect(inferDefaultVariant("anthropic", ["low", "high"])).toBe("high");
    expect(inferDefaultVariant("openai", ["low", "medium", "high"])).toBe("medium");
    expect(inferDefaultAgent([{ name: "plan" }, { name: "build" }])).toBe("build");
  });
});
