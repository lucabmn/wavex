...
describe("isRecoverableThreadResumeError", () => {
  it("detects missing-thread errors", () => {
    expect(isRecoverableThreadResumeError(new Error("Thread thr_x not found"))).toBe(true);
    expect(isRecoverableThreadResumeError(new Error("unknown thread id"))).toBe(true);
  });

  it("rejects unrelated errors", () => {
    expect(isRecoverableThreadResumeError(new Error("rate limited"))).toBe(false);
    expect(isRecoverableThreadResumeError(new Error("network down"))).toBe(false);
  });
});
...
describe("mapCodexNotification", () => {
  ...
  it("maps reasoning summary deltas", () => {
    ...
    expect(mapped.events).toEqual([{ type: "reasoning.delta", text: "thinking…" }]);
  });
  ...
  it("maps turn completion and clears active turn", () => {
    ...
    expect(mapped.events).toEqual(
      expect.arrayContaining([{ type: "message.completed" }, { type: "reasoning.completed" }]),
    );
  });
...
describe("parseCodexModelList", () => {
  it("builds models with reasoning and service tier settings", () => {
    ...
    expect(models.map((m) => m.nativeId)).toEqual(["gpt-5.6-terra", "gpt-5.6-luna"]);
    ...
    expect(luna?.settings?.find((s) => s.id === "reasoningEffort")?.value).toBe("medium");
  });
});
...
describe("mapCodexNotification thread/tokenUsage/updated", () => {
  it("reports the last request and the window the app-server supplies", () => {
    ...
    expect(mapped.events).toEqual([{ type: "context", used: 42_000, window: 272_000 }]);
  });
  ...
  it("never uses `total`, which keeps climbing past the window", () => {
    ...
    expect(mapped.events).toEqual([{ type: "context", used: 10_000, window: 272_000 }]);
  });
