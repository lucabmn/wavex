...
describe("grok protocol", () => {
  ...
  it("sends text-only prompt blocks", () => {
    expect(grokPromptBlocks("  hello  ")).toEqual([{ type: "text", text: "hello" }]);
  ...
  it("puts global flags before agent and stdio last", () => {
    ...
    expect(grokTextSpawnArgs()[grokTextSpawnArgs().length - 1]).toBe("stdio");
  ...
  it("parks supervised permissions and auto-allows full access", () => {
    ...
    expect(pickAutoOption("auto-accept-edits", "edit", options)).toBe("allow-once");
  ...
  it("parses grok models text output", () => {
    ...
    expect(models.map((model) => model.nativeId)).toEqual(["grok-4.6", "grok-4.5"]);
  });
