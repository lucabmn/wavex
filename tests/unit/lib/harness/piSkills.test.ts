...
vi.mock("@/lib/harness/piClient", () => ({
  PiRpc: class {
    ...
    constructor(_sessionId: string, onFrame: (record: Record<string, unknown>) => void) {
...
beforeEach(() => {
  ...
  mocks.watchChild.mockImplementation(
    (_id: string, _onLine: (line: string) => void, onExit: (code: number | null) => void) => {
      mocks.onExit = onExit;
    },
  );
});
...
describe("discoverPiSkills", () => {
  ...
  it("uses a unique child id for each probe", async () => {
    ...
    await discoverPiSkills("/repo");
    expect(mocks.spawnChild.mock.calls[0]![0]).not.toBe(mocks.spawnChild.mock.calls[1]![0]);
  });
});
