...
describe("fx protocol", () => {
  ...
  it("sends text-only prompt blocks", () => {
    expect(fxPromptBlocks("  hello  ")).toEqual([{ type: "text", text: "hello" }]);
  ...
  it("prefers allow_always so fx stops re-asking", () => {
    expect(autoPermissionOption("supervised", ["allow_once", "allow_always", "reject_once"])).toBe(
      "allow_always",
    );
  });

  it("picks allow/reject option ids from ACP permission options", () => {
    expect(permissionOptionId("allow", ["allow_once", "reject_once"])).toBe("allow_once");
    expect(permissionOptionId("deny", ["allow-once", "reject-once"])).toBe("reject-once");
  });
  ...
  it("parses a text model list when json is missing", () => {
    ...
    expect(models.map((model) => model.nativeId)).toEqual(["zai/glm-5.2-fast", "openai/gpt-5.4"]);
  });

  it("adds the TUI-selected status model when the list command omits it", () => {
    const listed = modelsFromFxOutput(JSON.stringify({ ids: ["zai/glm-4.7", "openai/gpt-5.2"] }));
    const active = modelFromFxStatusOutput(
      JSON.stringify({ kind: "status", model: "zai/glm-5.2" }),
    );
    expect(mergeFxCatalogModels(listed, active).map((model) => model.nativeId)).toEqual([
      "zai/glm-4.7",
      "openai/gpt-5.2",
      "zai/glm-5.2",
    ]);
