...
describe("inferShellIntent", () => {
  it("reads a file from cat / head / sed -n", () => {
    ...
    expect(inferShellIntent("sed -n '713,1200p' src/surfaces/AgentTranscript.tsx")).toEqual({
      verb: "Read",
  ...
  it("treats grep / rg as Find", () => {
    expect(
      inferShellIntent('grep -n "^function \\|^const .* = memo" src/surfaces/AgentTranscript.tsx'),
    ).toEqual({
      verb: "Find",
  ...
  it("treats file-mutating bash as Edit / Write", () => {
    ...
    expect(inferShellIntent("cat >> src/surfaces/transcriptActivity.ts <<'TS'")).toEqual({
      verb: "Edit",
...
describe("formatShellIntent", () => {
  it("prefers a display path passed in from the transcript", () => {
    expect(
      formatShellIntent({ verb: "Read", path: "/Users/me/proj/src/app.ts" }, "src/app.ts"),
