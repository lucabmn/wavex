...
describe("turnStatusFromResult", () => {
  it("treats aborted terminals as interrupted", () => {
    ...
    expect(turnStatusFromResult({ type: "result", subtype: "success" }).status).toBe("completed");
  });
});
...
describe("list_models catalog", () => {
  ...
  it("maps the picker catalog and drops default/disabled rows", () => {
    const models = modelsFromClaudeListModels(listModelsFromControlResponse(listed, "list_1"));
    ...
    ).toEqual(["low", "medium", "high", "xhigh", "max", "ultracode", "ultrathink"]);
...
describe("helpers", () => {
  ...
  it("classifies tools and todo plans", () => {
    ...
    expect(toolTitle("Task", { subagent_type: "explore" })).toBe("Explore subagent");
    expect(toolTitle("Bash", { command: "ls -la src" })).toBe("List src");
    expect(toolTitle("Skill", { skill: "code-review" })).toBe("Skill /code-review");
  ...
  it("drops request lifecycle status pings", () => {
    ...
    expect(statusTextFromSystem({ type: "system", subtype: "status" })).toBeUndefined();
  });
  ...
  it("ignores system messages that are not status or compact", () => {
    ...
    expect(statusTextFromSystem({ type: "assistant", message: "hello" })).toBeUndefined();
  });
});
...
describe("subagent messages", () => {
  ...
  it("parses task lifecycle frames for local agents", () => {
    ...
    ).toEqual([{ taskId: "t1", taskType: "local_agent", description: "Explore" }]);
    expect(
      parseToolProgress({
        type: "tool_progress",
        tool_use_id: "toolu_agent",
