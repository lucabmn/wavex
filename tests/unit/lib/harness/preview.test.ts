...
describe("extractToolPreview", () => {
  ...
  it("finds search queries in nested input", () => {
    expect(extractSearchQuery([{ arguments: { pattern: "busyHarness|busy.*tab" } }])).toBe(
      "busyHarness|busy.*tab",
    );
  });
...
describe("shell command titles", () => {
  it("pulls the command out of nested input bags", () => {
    expect(extractShellCommand({ args: { command: "git status -s" } })).toBe("git status -s");
    expect(extractShellCommand({ cmd: "pwd" })).toBe("pwd");
    expect(extractShellCommand({ command: ["npm", "test"] })).toBe("npm test");
    expect(extractShellCommand({}, { rawInput: {} }, { input: { command: "git status" } })).toBe(
      "git status",
    );
  });
  ...
  it("labels execute tools with the command, not the tool name", () => {
    ...
    expect(titleFromToolInput("bash", "execute", { command: "ls -la" })).toBe("ls -la");
    expect(
  ...
  it("does not substitute Claude's description for the command", () => {
    expect(titleFromToolInput("Bash", "execute", { description: "List files" })).toBe("Shell");
...
describe("skill titles", () => {
  it("labels a Skill tool with /name", () => {
    ...
    expect(extractSkillName({}, { rawInput: {} }, { skill: "code-review" })).toBe("code-review");
