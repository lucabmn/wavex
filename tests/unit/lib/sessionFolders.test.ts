...
describe("uniqueFolderName", () => {
  it("uses New folder, then numbers", () => {
    ...
    expect(uniqueFolderName([folder("a", ["s"], { name: "New folder" })])).toBe("New folder 2");
    expect(
      uniqueFolderName([
...
describe("buildSessionList", () => {
  it("places folders above pinned ungrouped sessions", () => {
    ...
    const entries = buildSessionList(sessions, folders, ungroupedSessions(sessions, folders));
    expect(
      entries.map((entry) =>
        entry.kind === "folder"
          ? entry.folder.name
  ...
  it("sorts members inside a folder by pin then recency", () => {
    ...
    expect(entries[0].sessions.map((row) => row.id)).toEqual(["pinned", "new", "old"]);
  });
...
describe("folder mutations", () => {
  ...
  it("stores a custom hex and prefers it over a palette index", () => {
    ...
    expect(folderShellFill(undefined, "#3b82f6")).toMatch(/color-mix\(in srgb, #3b82f6 18%/);
...
describe("applySessionListDrop", () => {
  ...
  it("ignores a drop onto itself or a sibling in the same folder", () => {
    ...
    expect(applySessionListDrop(folders, "a", { kind: "session", id: "a" }).folders).toBe(folders);
    expect(applySessionListDrop(folders, "a", { kind: "session", id: "b" }).folders).toBe(folders);
  });
...
describe("pruneSessionFolders", () => {
  it("drops unknown members and empty folders, keeping the same array when nothing changed", () => {
    const folders = [folder("keep", ["a", "gone"]), folder("empty", ["missing"])];
...
describe("session folder persistence", () => {
  ...
  it("round-trips a custom folder color and prefers it over a palette index", () => {
    const folders = [folder("g", ["a"], { name: "Work", customColor: "#AABBCC" })];
    saveSessionFolders("/tmp/project", folders);
  ...
  it("drops an invalid custom folder color on load", () => {
    saveSessionFolders("/tmp/project", [folder("g", ["a"], { name: "Work", customColor: "red" })]);
