...
describe("model settings memory", () => {
  ...
  it("drops values the new model does not support", () => {
    expect(mergeModelSettings(haiku, { effort: "xhigh", fast: "true" })).toEqual({
      thinking: "false",
    });
  });
  ...
  it("uses the current session when nothing has been stored yet", () => {
    expect(preferredModelSettings(opus, { effort: "xhigh", fast: "true" })).toEqual({
      effort: "xhigh",
      fast: "true",
    });
  });
...
describe("model picker tabs", () => {
  const available = (id: HarnessId) => id === "claude" || id === "fx" || id === "cursor";

  it("starts with favorites then installed providers", () => {
    expect(modelPickerTabs(available)).toEqual(["favorites", "claude", "cursor", "fx"]);
  });
