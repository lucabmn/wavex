...
import {
  ...
  inboxFetchState,
  pruneInboxFilters,
} from "../../../src/lib/inboxFilters";
import type { InboxItem } from "../../../src/lib/githubTasks";
...
describe("filterInboxByProject", () => {
  it("hides selected projects", () => {
    ...
    expect(filterInboxByProject(rows, ["/tmp/web/"]).map((row) => row.number)).toEqual([2]);
  });

  it("keeps items that are not tied to a folder", () => {
    const rows = [
      item({ number: 1, updatedAt: "2026-08-27T10:00:00Z", projectPath: "/tmp/web" }),
      item({ number: 9, projectPath: "", updatedAt: "2026-08-27T10:00:00Z" }),
    ];
    expect(filterInboxByProject(rows, ["/tmp/web"]).map((row) => row.number)).toEqual([9]);
  });
});
...
describe("filterInboxByKind", () => {
  it("hides selected kinds", () => {
    ...
    expect(filterInboxByKind(rows, ["pr"]).map((row) => row.number)).toEqual([1]);
    expect(filterInboxByKind(rows, ["issue"]).map((row) => row.number)).toEqual([2]);
  });
});
...
describe("filterInboxByStatus", () => {
  ...
  it("keeps every item when no status is selected", () => {
    expect(
      filterInboxByStatus(rows, DEFAULT_INBOX_FILTERS.status).map((row) => row.number),
    ).toEqual([1, 2, 3, 4]);
  });
...
describe("filterInboxByTime", () => {
  ...
  it("keeps items updated today", () => {
    ...
    expect(filterInboxByTime(rows, "today", now).map((row) => row.number)).toEqual([1]);
  });
});
