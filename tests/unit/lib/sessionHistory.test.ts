...
describe("historyWithLiveSessions", () => {
  it("drops persisted sessions from other projects", () => {
    const history = [summary("a1", "/tmp/project-a"), summary("b1", "/tmp/project-b")];
  ...
  it("copies origin repo from sibling history and prefers the live branch", () => {
    ...
    const rows = historyWithLiveSessions(history, [session], "/tmp/agent-terminal", {
      repo: "agent-terminal",
      branch: "fix-gutter",
    });
...
describe("filterSessionsByArchive", () => {
  it("hides archived sessions by default", () => {
    ...
    expect(filterSessionsByArchive(rows, false).map((row) => row.id)).toEqual(["a1"]);
  });
  ...
  it("shows only archived sessions when filtered", () => {
    ...
    expect(filterSessionsByArchive(rows, true).map((row) => row.id)).toEqual(["a2"]);
  });
});

describe("filterSessionsByQuery", () => {
  it("returns all rows when the query is empty", () => {
    const rows = [summary("a1", "/tmp/project-a"), summary("a2", "/tmp/project-a")];
    expect(filterSessionsByQuery(rows, "  ").map((row) => row.id)).toEqual(["a1", "a2"]);
  });
  ...
  it("matches conversation titles", () => {
    ...
    expect(filterSessionsByQuery(rows, "sidebar").map((row) => row.id)).toEqual(["a1"]);
  });
  ...
  it("matches model and branch labels", () => {
    ...
    expect(filterSessionsByQuery(rows, "opus").map((row) => row.id)).toEqual(["a2"]);
    expect(filterSessionsByQuery(rows, "gutter").map((row) => row.id)).toEqual(["a2"]);
  });
});

describe("replaceProjectHistory", () => {
  it("swaps one project's rows and keeps the others cached", () => {
    const current = [summary("a1", "/tmp/project-a", 3), summary("b1", "/tmp/project-b", 2)];
...
describe("mergeProjectHistorySummary", () => {
  it("merges into its own project and leaves other projects cached", () => {
    const current = [summary("a1", "/tmp/project-a", 1), summary("b1", "/tmp/project-b", 2)];
    const next = mergeProjectHistorySummary(current, summary("a1", "/tmp/project-a", 9));
  ...
  it("does not leave a duplicate behind when a session changes project", () => {
    const current = [summary("a1", "/tmp/project-a", 1), summary("b1", "/tmp/project-b", 2)];
    const next = mergeProjectHistorySummary(current, summary("a1", "/tmp/project-b", 9));
    expect(next.map((row) => row.id).sort()).toEqual(["a1", "b1"]);
...
describe("pinned sessions", () => {
  it("keeps pinned sessions above newer unpinned ones", () => {
    ...
    const next = mergeHistorySummary(current, summary("new", "/tmp/project-a", 30));
  ...
  it("preserves pin when an incoming summary omits it", () => {
    ...
    const next = mergeHistorySummary(current, summary("pin", "/tmp/project-a", 9));
