...
import { formatFileSize, isImagePath, sniffImageMime } from "@/lib/files/filePreview";
...
describe("sniffImageMime", () => {
  it("identifies each supported format by magic number", () => {
    ...
    expect(sniffImageMime(bytes(0x52, 0x49, 0x46, 0x46, 1, 2, 3, 4, 0x57, 0x45, 0x42, 0x50))).toBe(
      "image/webp",
    );
    expect(
      sniffImageMime(bytes(0, 0, 0, 0x20, 0x66, 0x74, 0x79, 0x70, 0x61, 0x76, 0x69, 0x66)),
    ).toBe("image/avif");
  });
  ...
  it("refuses content that is not a supported image", () => {
    ...
    expect(
      sniffImageMime(bytes(0, 0, 0, 0x20, 0x66, 0x74, 0x79, 0x70, 0x69, 0x73, 0x6f, 0x6d)),
    ).toBeNull();
  });
});
