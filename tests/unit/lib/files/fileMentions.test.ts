...
import { loadProjectFiles } from "@/lib/files/fileIndex";
import {
...
vi.mock("@/lib/files/fileIndex", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/files/fileIndex")>();
  return {
    ...actual,
...
describe("replaceMentionToken", () => {
  it("inserts @label and a trailing space", () => {
    expect(replaceMentionToken("@Comp", { start: 0, end: 5, query: "Comp" }, "Composer.tsx")).toBe(
      "@Composer.tsx ",
    );
    expect(replaceMentionToken("x @a y", { start: 2, end: 4, query: "a" }, "App.tsx")).toBe(
      "x @App.tsx y",
    );
  });
});
...
describe("fileMentionParts", () => {
  ...
  it("ignores file mentions inside Markdown blockquotes", () => {
    ...
    expect(fileMentionsInText(text, index.labels).map((hit) => hit.label)).toEqual([
      "Composer.tsx",
    ]);
    expect(fileMentionParts(text, index.labels).filter((part) => part.file)).toHaveLength(1);
  });
});
...
describe("fileMentionsInText", () => {
  it("collects each referenced file once", () => {
    ...
    expect(hits.map((hit) => hit.label)).toEqual(["Composer.tsx", "apps/web/src/App.tsx"]);
  });
});
...
describe("withMentionDirectories", () => {
  ...
  it("keeps parent folders that contain spaces", () => {
    ...
    expect(entries.some((file) => file.relative === "My Photos" && file.isDir)).toBe(true);
  });
  ...
  it("inserts a spaced folder as a normal @ token", () => {
    ...
    const folder = [...photos.labels.values()].find((file) => file.relative === "My Photos");
...
describe("applyFileMentionsToTurn", () => {
  ...
  it("does not invent a path from a token that is not in the index", async () => {
    const out = await applyFileMentionsToTurn("look at @notes/read-me.md~2", "/p");
