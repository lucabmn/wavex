...
describe("releaseNotesForVersion", () => {
  it("extracts only the requested release", () => {
    ...
    expect(releaseNotesTitle(release!.source.version)).toBe("What's new in wavex 0.1.2");
  ...
  it.each(["## [0.1.2]\n\nUndated.", "## [0.1.2] - 2026-08-31\n\nDated."])(
    ...
    (changelog) => {
      expect(releaseNotesForVersion("0.1.2", changelog)?.markdown).toBe(changelog);
    },
  );
  ...
  it("escapes the requested version before matching", () => {
    expect(releaseNotesForVersion("0.1.2+test", "## [0.1.2+test]\n\nExact.")).toEqual({
...
describe("releaseNotesMarkdown", () => {
  it("resolves a stored source against the bundled changelog shape", () => {
    const release = releaseNotesForVersion("0.1.2", fixture);
    expect(releaseNotesMarkdown(release!.source, fixture)).toBe(release?.markdown);
  });
});
...
describe("formatReleaseDate", () => {
  ...
  it.each(["soon", "2026-13-01", "2026-09-1"])("leaves invalid date %j alone", (value) => {
