...
import { leaf, leafIds, newTab, type WorkspaceTab } from "../../../src/lib/layout";
...
import {
  ...
  workspaceTabProject,
} from "../../../src/lib/workspaceTabGroups";
...
describe("filterTabsForProject", () => {
  it("keeps only tabs that belong to the project", () => {
    ...
    expect(filterTabsForProject(tabs, sessions, "/tmp/beta").map((tab) => tab.id)).toEqual([
      "t2",
      "t3",
    ]);
  });
...
describe("planWorkspaceTabClose", () => {
  ...
  it("uses the global neighbor for a projectless tab", () => {
    const projectlessSessions = [...sessions, session("blank1", "~"), session("blank2", "~")];
    expect(
      planWorkspaceTabClose({
