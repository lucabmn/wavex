...
describe("noteTitle", () => {
  ...
  it("falls back to the first prose line", () => {
    expect(noteTitle("Ship the notes overlay first.")).toBe("Ship the notes overlay first.");
  });
...
describe("notePreview", () => {
  it("skips the title heading", () => {
    expect(notePreview("# Auth\n\nKeep it global.", "Auth")).toBe("Keep it global.");
  });
});

describe("noteSourceProject", () => {
  it("uses the folder name", () => {
    expect(noteSourceProject("/Users/me/code/agent-terminal")).toBe("agent-terminal");
  });
...
describe("note mentions", () => {
  ...
  it("maps notes to mention files", () => {
    const files = notesAsProjectFiles([note({ id: "abc", slug: "auth", title: "Auth" })]);
    ...
    expect(noteMentionLabel(note({ id: "abc", slug: "auth", title: "Auth" }))).toBe("note/auth");
  });
});
...
describe("rankNoteFiles", () => {
  ...
  it("keeps recency order without a query", () => {
    expect(rankNoteFiles(notes, "").map((file) => file.path)).toEqual(["note:b", "note:a"]);
  });

  it("fuzzy-matches titles", () => {
    expect(rankNoteFiles(notes, "plan").map((file) => file.path)).toEqual(["note:b"]);
  });
});

describe("noteMentionLabel", () => {
  it("prefixes the slug", () => {
    expect(noteMentionLabel(note({ id: "n", slug: "auth", title: "Auth" }))).toBe("note/auth");
  });
});
