...
function block(role: Block["role"], approval?: Block["approval"]): Block {
  return {
    ...
    ...(role === "tool" ? { tool: { title: "Run command", kind: "execute" } } : {}),
    ...(approval ? { approval } : {}),
  };
}
...
describe("isSessionConversationFocused", () => {
  it("is true only when the session pane is focused on the active tab", () => {
    ...
    expect(isSessionConversationFocused(session.id, tab.id, [tab], true)).toBe(true);
    expect(isSessionConversationFocused(session.id, tab.id, [tab], false)).toBe(false);
    expect(isSessionConversationFocused("other", tab.id, [tab], true)).toBe(false);
  });
});
...
describe("hiddenApprovalNotices", () => {
  it("omits approvals that are already in the focused conversation", () => {
    ...
    expect(
      hiddenApprovalNotices([visible, hidden], visibleTab.id, [visibleTab, hiddenTab], true).map(
  ...
  it("shows approvals in another pane on the same tab", () => {
    ...
    expect(
      hiddenApprovalNotices([left, right], tab.id, [tab], true).map((notice) => notice.sessionId),
    ).toEqual([left.id]);
  });
});
