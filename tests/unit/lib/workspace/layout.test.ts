...
describe("openChangesTab", () => {
  ...
  it("drops per-file review tabs in the same pane", () => {
    const cwd = "/repo";
    const withReview = openEditorTab(newTab("session-a"), newFileTab("/repo/a.ts", cwd, true));
...
describe("newReleaseNotesWorkspaceTab", () => {
  it("creates a projectless editor-only workspace tab", () => {
    ...
    expect(layoutLeaves(tab.layout).map((pane) => pane.id)).toEqual([tab.editorPanes[0]?.id]);
    expect(tab.focusedId).toBe(tab.editorPanes[0]?.id);
  });

  it("deduplicates release files by version", () => {
    const first = newReleaseNotesWorkspaceTab({ version: "0.1.23" }).editorPanes[0]!.files[0]!;
    const second = newReleaseNotesWorkspaceTab({ version: "0.1.23" }).editorPanes[0]!.files[0]!;
...
describe("openTerminalTab", () => {
  it("occupies a session pane instead of splitting a leftover chat", () => {
    ...
    expect(layoutLeaves(next.layout).map((pane) => pane.id)).toEqual([next.terminalPanes[0]?.id]);
  ...
  it("splits a terminal pane below the session when no terminal pane exists", () => {
    ...
    expect(leaves.map((pane) => pane.id)).toEqual(["session-a", next.terminalPanes[0]?.id]);
  ...
  it("keeps terminals out of the file pane tab strip", () => {
    const withFile = openEditorTab(newTab("session-a"), newFileTab("/repo/App.tsx", "/repo"));
    ...
    expect(next.editorPanes[0]?.files.map((file) => file.path)).toEqual(["/repo/App.tsx"]);
    expect(next.terminalPanes).toHaveLength(1);
    expect(next.terminalPanes[0]?.files.map((file) => file.id)).toEqual([first.id, extra.id]);
...
describe("closeLeaf", () => {
  it("keeps a file pane when the last chat is closed", () => {
    ...
    expect(layoutLeaves(next!.layout).map((pane) => pane.id)).toEqual([next!.editorPanes[0]?.id]);
...
describe("openEditorTab", () => {
  it("does not open files into a terminal pane", () => {
    const terminal = openTerminalTab(newTab("session-a"), newTerminalFile("/repo"));
...
describe("newTerminalWorkspaceTab", () => {
  it("creates a tab whose only leaf is the terminal pane", () => {
    ...
    expect(layoutLeaves(tab.layout).map((pane) => pane.id)).toEqual([tab.terminalPanes[0]?.id]);
...
describe("isolateTerminalPanes", () => {
  it("splits mixed file and terminal tabs into separate panes", () => {
    ...
    expect(next.editorPanes[0]?.files.map((entry) => entry.id)).toEqual([file.id]);
    expect(next.terminalPanes[0]?.files.map((entry) => entry.id)).toEqual([terminal.id]);
    expect(next.focusedId).toBe(next.terminalPanes[0]?.id);
    expect(layoutLeaves(next.layout)).toHaveLength(3);
  });
});
...
describe("movePane", () => {
  ...
  it("nests a pane under one column of a row", () => {
    const row = splitPane(splitPane(leaf("a"), "a", "right", "b"), "b", "right", "c");
