...
describe("collectWorkspaceSnapshot", () => {
  it("stores tabs, stubs, and the focused tab — not transcripts", () => {
    ...
    expect(snapshot.tabs[0]?.editorPanes[0]?.files[0]?.path).toBe("/tmp/a/README.md");
...
describe("parseWorkspaceSnapshot", () => {
  ...
  it.each([
    ...
    expect(workspace?.sessions.some((session) => session.id === invalidPaneId)).toBe(false);
  });
});
...
describe("hydrateWorkspaceSnapshot", () => {
  it("reopens splits, file panes, and stored transcripts", () => {
    ...
    const snapshot = collectWorkspaceSnapshot([tab], [left, right], "t1", "/tmp/a");
    const loaded = new Map([
      [
        "s1",
        {
          ...left,
          blocks: [...left.blocks, { id: "a1", role: "assistant" as const, text: "stored" }],
        },
      ],
    ]);
    ...
    expect(workspace?.tabs[0]?.editorPanes[0]?.files[0]?.path).toBe("/tmp/a/src/lib.rs");
  ...
  it("marks in-flight chats interrupted and adds a tab if they were parked", () => {
    ...
    expect(resumed?.blocks.some((block) => block.text === INTERRUPT_MESSAGE)).toBe(true);
  });
  ...
  it("restores a panel that only ever held a page", () => {
    const dock = {
      ...createProjectDock("/tmp/a", { surface: "browser" as const }),
  ...
  it("reads a dock written before the panel had surfaces as terminals", () => {
    ...
    const legacy = {
      projectPath: "/tmp/a",
      pane: { id: "p1", files: [term], activeFileId: term.id },
      side: "bottom",
      size: 220,
      open: true,
    };
