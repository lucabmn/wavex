...
describe("tabCommand", () => {
  it("opens a terminal pane with cmd-backtick", () => {
    expect(tabCommand(key({ key: "`", code: "Backquote", metaKey: true }))).toBe("new-terminal");
  });

  it("opens a terminal workspace tab with shift-cmd-backtick", () => {
    expect(tabCommand(key({ key: "~", code: "Backquote", metaKey: true, shiftKey: true }))).toBe(
      "new-terminal-tab",
    );
  });
  ...
  it("keeps existing tab chrome bindings", () => {
    ...
    expect(tabCommand(key({ key: "t", metaKey: true, altKey: true }))).toBe("close-others");
    expect(tabCommand(key({ key: "t", ctrlKey: true, altKey: true }))).toBe("close-others");
    expect(tabCommand(key({ key: "d", metaKey: true }))).toBe("split-right");
    expect(tabCommand(key({ key: "j", metaKey: true }))).toBe("toggle-terminal");
  });

  it("walks tab visit history with cmd-brackets", () => {
    expect(tabCommand(key({ key: "[", code: "BracketLeft", metaKey: true }))).toBe("back");
    expect(tabCommand(key({ key: "]", code: "BracketRight", metaKey: true }))).toBe("forward");
  });

  it("keeps shift-cmd-brackets as adjacent tab cycle", () => {
    expect(tabCommand(key({ key: "{", code: "BracketLeft", metaKey: true, shiftKey: true }))).toBe(
      "prev",
    );
    expect(tabCommand(key({ key: "}", code: "BracketRight", metaKey: true, shiftKey: true }))).toBe(
      "next",
    );
  });
});
...
describe("shouldStopFocusedTurnOnEscape", () => {
  const escape = (partial: Partial<KeyboardEvent> = {}) => key({ key: "Escape", ...partial });

  it("stops a busy focused agent turn on plain Escape", () => {
    expect(
      shouldStopFocusedTurnOnEscape(escape(), { inTerminal: false, focusedSessionBusy: true }),
    ).toBe(true);
  });
  ...
  it("leaves terminal Escape and idle sessions alone", () => {
    expect(
      shouldStopFocusedTurnOnEscape(escape(), { inTerminal: true, focusedSessionBusy: true }),
    ).toBe(false);
    expect(
      shouldStopFocusedTurnOnEscape(escape(), { inTerminal: false, focusedSessionBusy: false }),
    ).toBe(false);
  });
...
describe("focusedBusyAgentSessionId", () => {
  ...
  it("returns only the busy agent session in the exact active tab", () => {
    expect(focusedBusyAgentSessionId("tab-a", tabs, sessions, false)).toBe("session-a");
    expect(focusedBusyAgentSessionId("missing", tabs, sessions, false)).toBeNull();
  });

  it("does not stop through diff, terminal-dock, editor, or idle focus", () => {
    expect(
      focusedBusyAgentSessionId("tab-a", [{ ...tabs[0], diffFocused: true }], sessions, false),
    ).toBeNull();
    ...
    expect(
      focusedBusyAgentSessionId("tab-a", tabs, [{ id: "session-a", busy: false }], false),
    ).toBeNull();
  });
});
