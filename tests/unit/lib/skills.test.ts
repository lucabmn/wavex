...
describe("replaceSlashToken", () => {
  it("inserts an exact invocation and a trailing space", () => {
    ...
    expect(replaceSlashToken("x /r y", { start: 2, end: 4, query: "r" }, "review-pr")).toBe(
      "x /review-pr y",
    );
    expect(replaceSlashToken("/arch", { start: 0, end: 5, query: "arch" }, "skill:architect")).toBe(
      "/skill:architect ",
    );
  });
});
...
describe("skillNamesInText", () => {
  it("collects unique /skill tokens", () => {
    expect(skillNamesInText("/create-skill write a deploy skill")).toEqual(["create-skill"]);
...
describe("mergeCatalog", () => {
  it("lets .agents win, then wavex create-skill, then provider skills", () => {
    ...
    expect(catalog.find((s) => s.name === "review-pr")?.description).toBe("from agents");
    expect(catalog.find((s) => s.name === "create-skill")).toEqual(BUILTIN_CREATE_SKILL);
...
describe("rankSkills", () => {
  it("puts create-skill first when the query is empty", () => {
    ...
    expect(ranked.map((s) => s.name)).toEqual(["create-skill", "cursor-only", "review-pr"]);
  });
  ...
  it("ranks native Pi rows with project skills", () => {
    ...
    expect(ranked.map((skill) => skill.name)).toEqual(["architect", "pi-file", "review-pr"]);
  });
  ...
  it("keeps every Pi result when the composer removes the default cap", () => {
    ...
    expect(rankSkills(rows, "skill-74", Number.POSITIVE_INFINITY)[0]?.name).toBe("skill-74");
  });
});
