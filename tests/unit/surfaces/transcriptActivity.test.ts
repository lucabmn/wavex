...
describe("groupTurnItems", () => {
  it("keeps consecutive shell calls in one activity stack", () => {
    const items = groupTurnItems([shell("a"), shell("b"), shell("c", "pending")]);
  ...
  it("folds prose between tool calls in and leaves the final answer out", () => {
    ...
    expect(items[1].blocks.map((block) => block.id)).toEqual(["a1", "a", "a2", "b"]);
  ...
  it("keeps the trailing run of prose blocks out of the stack", () => {
    ...
    expect(items.map((item) => item.type)).toEqual(["activity", "block", "block"]);
  });
...
describe("buildActivityPhases", () => {
  it("groups a run of calls under the line that introduced it", () => {
    ...
    expect(phases[0].steps.map((block) => block.id)).toEqual(["s1", "r1", "r2"]);
  ...
  it("folds a lone uninvited call into the group before it", () => {
    ...
    expect(phases[0].steps.map((block) => block.id)).toEqual(["e1", "r1", "e2"]);
  });

  it("keeps a group the agent announced out of that fold", () => {
    const phases = buildActivityPhases([read("r1"), note("n1", "Now the edit."), edit("e1")]);
    expect(phases).toHaveLength(2);
  ...
  it("keeps a second paragraph as a step rather than a group of its own", () => {
    const phases = buildActivityPhases([note("n1", "First."), note("n2", "Second."), read("r1")]);
    expect(phases).toHaveLength(1);
    expect(phases[0]).toMatchObject({
      kind: "research",
      headline: { id: "n1" },
  ...
  it("keeps reasoning inside the group instead of titling it", () => {
    const phases = buildActivityPhases([thought("t1"), search("s1"), thought("t2"), search("s2")]);
    expect(phases).toHaveLength(1);
    expect(phases[0]).toMatchObject({ kind: "research", headline: undefined });
    expect(phases[0].steps.map((block) => block.id)).toEqual(["t1", "s1", "t2", "s2"]);
  });
  ...
  it("moves a thought at the end of a group into the group it introduced", () => {
    ...
    expect(phases[1].steps.map((block) => block.id)).toEqual(["t1", "e1", "e2"]);
  });
...
describe("activityPhaseTitle", () => {
  ...
  it("uses the agent's own line when it wrote one", () => {
    expect(
      title([note("n1", "**Found it** — the tokens live in `globals.css`."), read("r1")]),
    ).toBe("Found it — the tokens live in globals.css.");
  });

  it("says what the calls add up to, in the tense of the moment", () => {
    expect(title([read("r1", "a.ts"), read("r2", "b.ts")], true)).toBe("Reading 2 files");
    expect(title([read("r1", "a.ts"), read("r2", "b.ts")])).toBe("Read 2 files");
    ...
    expect(title([edit("e1", "a.ts"), edit("e2", "b.ts")])).toBe("Edited 2 files");
...
describe("lastActivityIndex", () => {
  ...
  it("returns -1 for a turn that ran no tools", () => {
    expect(lastActivityIndex(groupTurnItems([{ id: "a", role: "assistant", text: "Hi." }]))).toBe(
      -1,
    );
  });
});
...
describe("nestedScrollAbsorbsWheel", () => {
  ...
  it("lets the parent handle the wheel when the list does not overflow", () => {
    expect(
      nestedScrollAbsorbsWheel({ scrollTop: 0, scrollHeight: 80, clientHeight: 80 }, -20),
    ).toBe(false);
  });
  ...
  it("releases the wheel at the edges so the transcript can take over", () => {
    expect(nestedScrollAbsorbsWheel({ ...overflowing, scrollTop: 0 }, -20)).toBe(false);
    expect(nestedScrollAbsorbsWheel({ ...overflowing, scrollTop: 120 }, 20)).toBe(false);
  });
});

describe("proseSummary", () => {
  it("reduces a paragraph to one plain line", () => {
    expect(proseSummary("**Full checks pass** — `cargo fmt` and 134 tests.\n\nBumping:")).toBe(
